...
function getDiceBearAvatarUrlServer(name, randomSeed = '') {
    const seed = encodeURIComponent(name ? name.toLowerCase() : 'default' + randomSeed); // Added null check for name
    ...
    try {
        ...
        let html = `
            ...
            <head>
                ...
                <style>
                    ...
                    .feedback-info h4 { margin: 0; font-size: 1.4em; color: #FFD700; text-transform: uppercase; display: flex; align-items: center; gap: 8px; word-break: break-word; }
                    ...
                    .feedback-body { font-size: 1em; color: #BDC3C7; line-height: 1.6; margin-bottom: 15px; flex-grow: 1; overflow-y: auto; word-break: break-word; }
                    ...
                    .reply-content-wrapper { flex-grow: 1; word-break: break-word; } .reply-admin-name { font-weight: bold; color: #9B59B6; display: inline; margin-right: 5px; }
                    ...
                    .admin-custom-modal p { margin-bottom: 25px; font-size: 1.1em; line-height: 1.6; color: #ccc; word-break: break-word; }
        ...
        if (feedbacks.length === 0) {
            ...
            feedbacks.forEach(fb => {
                // Null/undefined checks for names before using charAt or displaying
                const displayName = (fb.name && typeof fb.name === 'string') ? fb.name : 'Anonymous';
                const displayAvatarAlt = (fb.name && typeof fb.name === 'string') ? displayName.charAt(0).toUpperCase() : 'A';
                
                const originalDisplayName = (fb.originalContent && fb.originalContent.name && typeof fb.originalContent.name === 'string') ? fb.originalContent.name : 'N/A';
                // Avatar for original content usually uses the same current avatar, alt might change if name changed
                // const originalAvatarAlt = (fb.originalContent && fb.originalContent.name && typeof fb.originalContent.name === 'string') ? originalDisplayName.charAt(0).toUpperCase() : 'A';
                        ...
                        <div class="feedback-card-inner">
                            <div class="feedback-card-front">
                                <div class="feedback-header">
                                    <div class="feedback-avatar"><img src="${fb.avatarUrl || getDiceBearAvatarUrlServer(displayName)}" alt="${displayAvatarAlt}"></div>
                                    <div class="feedback-info">
                                        <h4>${displayName} ${fb.isEdited ? '<span class="edited-admin-tag">EDITED</span>' : ''}</h4>
                                ...
                                <div class="action-buttons">
                                    ...
                                    <button class="change-avatar-btn" onclick="tryChangeAvatar('${fb._id}', '${displayName}')">AVATAR BADAL!</button>
                                </div>
                ...
                if (fb.isEdited && fb.originalContent) {
                    html += `
                            <div class="feedback-card-back">
                                <div class="feedback-header">
                                    <div class="feedback-avatar"><img src="${fb.avatarUrl || getDiceBearAvatarUrlServer(originalDisplayName)}" alt="${(originalDisplayName !== 'N/A') ? originalDisplayName.charAt(0).toUpperCase() : 'A'}"></div>
                                    <div class="feedback-info">
                                        <h4>ORIGINAL: ${originalDisplayName}</h4>
                                        <div class="rating">${'★'.repeat(fb.originalContent.rating || 0)}${'☆'.repeat(5 - (fb.originalContent.rating || 0))}</div>
                                    </div>
                                </div>
                                <div class="feedback-body"><p>${fb.originalContent.feedback || 'N/A'}</p></div>
                                <div class="feedback-date">Posted: ${fb.originalContent.timestamp ? new Date(fb.originalContent.timestamp).toLocaleString() : 'N/A'}</div>
        ...
        html += `
                ...
                <script>
                    ...
                    async function tryPostReply(feedbackId, textareaId) {
                        ...
                        showAdminModal('confirm', 'PAKKA BHEJNA HAI?', \`REPLY (shuru ke 100 अक्षर): "\${replyText.substring(0,100)}..."\`, async (confirmed) => {
                            if (confirmed) {
                                try {
        ...
        `;
        res.send(html);
    } catch (error) { // Yeh error ab upar wale try-catch mein ayega agar HTML generation mein problem hui
        console.error('SAALA! ADMIN PANEL GENERATE KARTE WAQT FAT GAYI:', error); // Specific error log
        res.status(500).send(`SAALA! ADMIN PANEL KI FATTI HAI! ERROR: ${error.message}`); // User-friendly error
    }
});

// ... BAAKI ROUTES (DELETE, REPLY, CHANGE-AVATAR, LISTEN) PEHLE JAISE HI HAIN ...
...
app.put('/api/admin/feedback/:id/change-avatar', authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        const userName = feedbackToUpdate.name; // Use existing name for consistency
        const newAvatarUrl = getDiceBearAvatarUrlServer(userName, Date.now().toString());
        // Update all feedbacks with the same name to ensure consistent avatar, or just this one.
        // For now, updating all feedbacks by this user:
        await Feedback.updateMany({ name: userName }, { $set: { avatarUrl: newAvatarUrl } });
        // If only this one:
        // feedbackToUpdate.avatarUrl = newAvatarUrl;
