...
app.get('/admin-panel-nobita', authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        const nobitaAvatarUrl = 'https://i.ibb.co/FsSs4SG/creator-avatar.png'; // Assuming this is your NobiBot avatar URL

        let html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>ADMIN PANEL: NOBITA'S COMMAND CENTER</title><link href="https://fonts.googleapis.com/css2?family=Roboto:wght@400;700&display=swap" rel="stylesheet"><style>
            /* General Styles */
            ...
            .nobibot-icon {
                ...
                border-radius: 50%;
                overflow: hidden;
                box-shadow: 0 0 15px rgba(255,215,0,0.7), 0 0 25px rgba(255,215,0,0.5); /* Glowing effect */
                animation: pulse 2s infinite alternate; /* Pulsing animation */
                z-index: 1000;
                border: 3px solid #FFD700;
                background-color: #1A1A2E; /* Background to fill if image is smaller */
            }
            .nobibot-icon img {
                width: 100%;
                height: 100%;
        ...
        if (feedbacks.length === 0) {
            ...
            for (const fb of feedbacks) {
                ...
                // Safely get email from populated userId, if available
                const userEmail = (fb.userId && fb.userId.email) ? fb.userId.email : '';

                if (fb.userId) {
                   userTag = fb.userId.loginMethod === 'google' ? `<span class="google-user-tag" title="Google User (${userEmail})">G</span>` : `<span class="email-user-tag" title="Email User (${userEmail})">E</span>`;
                   userDisplayName = fb.userId.name || fb.name;
                   userEmailDisplay = userEmail ? `<small>(${userEmail})</small>` : '';
