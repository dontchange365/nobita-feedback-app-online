...
const feedbackSchema = new mongoose.Schema({
  ...
  avatarUrl: { type: String },
  userIp: { type: String },
  userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User', default: null }, // MODIFIED: Made userId optional for guests
...
app.post('/api/auth/request-email-verification', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (user.loginMethod === 'google') return res.status(400).json({ message: "Google accounts do not require email verification." });
        if (user.isVerified) return res.status(200).json({ message: "Your email is already verified." });
        const verificationToken = crypto.randomBytes(32).toString('hex');
...
app.post('/api/user/change-password', authenticateToken, isEmailVerified, async (req, res) => {
    ...
    try {
        const user = await User.findById(userId);
        if (!user) return res.status(404).json({ message: 'User not found.' });
        if (user.loginMethod === 'google') return res.status(400).json({ message: 'Password for Google-linked accounts cannot be changed here.' });
        if (!user.password) return res.status(400).json({ message: 'Your account does not have a password set. Please use the password reset feature.' });
    ...
    try {
        const result = await new Promise((resolve, reject) => {
            cloudinary.uploader.upload_stream({
                ...
                transformation: [ { width: 150, height: 150, crop: "fill", gravity: "face", radius: "max" }, { quality: "auto:eco" } ]
            }, (error, result) => {
                if (error) { console.error('Cloudinary upload error:', error); return reject(new Error(error.message)); }
...
app.get('/api/feedbacks', async (req, res) => {
    try {
        const allFeedbacks = await Feedback.find().populate({ path: 'userId', select: 'loginMethod isVerified email name' }).sort({ timestamp: -1 }); // Added 'name' to populate
...
// MODIFIED ROUTE for Feedback submission (Supports GUESTS)
app.post('/api/feedback', async (req, res) => {
    // 'name' from body is for guests. Logged-in users' names come from token.
    const { name: guestNameFromBody, feedback, rating } = req.body;
    ...
    let feedbackData = {
        ...
        userIp,
        isEdited: false,
        // name, avatarUrl, userId will be set based on auth status below
    };
    ...
    try {
        if (token) { // User is potentially logged in, token is present
            let decodedUserPayload;
            try {
                decodedUserPayload = jwt.verify(token, JWT_SECRET);
            } catch (jwtError) {
                // This catches invalid/expired tokens specifically from jwt.verify
            ...
            if (!loggedInUser) {
                // Token was valid (structurally) but the userId it contains doesn't exist in DB.
            ...
            // User is logged in and found in DB
        ...
        } else { // Guest user (no token provided)
            ...
            // Aapke server.js mein getDiceBearAvatarUrl function pehle se hai
            feedbackData.avatarUrl = getDiceBearAvatarUrl(guestNameFromBody, Date.now().toString()); // Generate guest avatar
            feedbackData.userId = null; // Explicitly set userId to null for guests
        }
        ...
        // General error catch (e.g., DB save error, unexpected issues)
        console.error("Feedback save error:", error);
        // Check for Mongoose validation error if userId was somehow still required and null
        if (error.name === 'ValidationError' && error.errors && error.errors.userId) {
             console.error("Mongoose validation error for userId:", error.errors.userId.message);
             return res.status(500).json({ message: 'Server error: Could not save feedback due to user ID issue. Ensure schema is updated as per instructions.' });
        }
        ...
        if (contentActuallyChanged) {
            if (!existingFeedback.originalContent) {
                existingFeedback.originalContent = {
                    name: existingFeedback.name, feedback: existingFeedback.feedback,
                    rating: existingFeedback.rating, timestamp: existingFeedback.timestamp
                };
            }
            existingFeedback.name = currentNameFromDb;
...
app.get('/admin-panel-nobita', authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        const redTickPath = '/images/red-tick.png';
        ...
        if (feedbacks.length === 0) {
            ...
            for (const fb of feedbacks) {
                let userTag = '', userDisplayName = fb.userId && fb.userId.name ? fb.userId.name : fb.name;
                ...
                if (fb.userId && typeof fb.userId === 'object') {
                    ...
                    if (fb.userId.isVerified) userTag += `<img src="${blueTickPath}" alt="Verified" title="Email Verified" style="width: 24px; height: 24px; vertical-align: middle; margin-left: 5px;">`;
                    else if (fb.userId.loginMethod === 'email') userTag += `<img src="${redTickPath}" alt="Unverified" title="Email Not Verified" style="width: 24px; height: 24px; vertical-align: middle; margin-left: 5px;">`;
                ...
                html += `<div class="feedback-card" id="card-${fb._id}"><div class="feedback-card-inner"><div class="feedback-card-front"><div class="feedback-header"><input type="checkbox" class="feedback-checkbox" value="${fb._id}"><div class="feedback-avatar"><img src="${fb.avatarUrl || getDiceBearAvatarUrl(userDisplayName)}" alt="${userDisplayName.charAt(0) || 'U'}"></div><div class="feedback-info"><h4>${userDisplayName} ${fb.isEdited ? '<span class="edited-admin-tag">EDITED</span>' : ''} ${userTag}</h4><small style="font-size:0.7em; color:#bbb; text-transform:none; margin-top: 5px; display: block;">${userEmailDisplay.replace(/[()]/g, '')}</small><div class="rating">${'★'.repeat(fb.rating)}${'☆'.repeat(5 - fb.rating)}</div><div class="user-ip">IP: ${fb.userIp || 'N/A'} | UserID: ${fb.userId ? (fb.userId._id ? fb.userId._id.toString() : fb.userId.toString()) : 'N/A'}</div></div></div><div class="feedback-body"><p>${fb.feedback}</p></div><div class="feedback-date">${fb.isEdited ? 'Last Edited' : 'Posted'}: ${new Date(fb.timestamp).toLocaleString()}${fb.isEdited && fb.originalContent ? `<br><small>Original: ${new Date(fb.originalContent.timestamp).toLocaleString()}</small>` : ''}</div><div class="action-buttons"><button class="delete-btn" onclick="tryDeleteFeedback('${fb._id}')">DELETE</button>${fb.userId && typeof fb.userId === 'object' && fb.userId.loginMethod === 'email' ? `<button class="change-avatar-btn" onclick="tryChangeUserAvatar('${fb.userId._id}', '${userDisplayName}')">AVATAR</button>` : ''}</div><div class="reply-section"><textarea id="reply-text-${fb._id}" placeholder="Admin reply..."></textarea><button class="reply-btn" onclick="tryPostReply('${fb._id}', 'reply-text-${fb._id}')">REPLY</button><div class="replies-display">${fb.replies && fb.replies.length > 0 ? '<h4>Replies:</h4>' : ''}${fb.replies.map(reply => `<div class="single-reply"><img src="${nobitaAvatarUrl}" alt="Admin" class="admin-reply-avatar-sm"><div class="reply-content-wrapper"><span class="reply-admin-name">${reply.adminName}:</span> ${reply.text}<span class="reply-timestamp">(${new Date(reply.timestamp).toLocaleString()})</span></div></div>`).join('')}</div></div>${fb.isEdited && fb.originalContent ? `<button class="flip-btn" onclick="flipCard('${fb._id}')">VIEW ORIGINAL</button>` : ''}</div>`;
                if (fb.isEdited && fb.originalContent) {
                    html += `<div class="feedback-card-back"><div class="feedback-header"><div class="feedback-avatar"><img src="${(fb.originalContent.avatarUrl || fb.avatarUrl)}" alt="Original"></div><div class="feedback-info"><h4>ORIGINAL: ${fb.originalContent.name}</h4><div class="rating">${'★'.repeat(fb.originalContent.rating)}${'☆'.repeat(5 - fb.originalContent.rating)}</div></div></div><div class="feedback-body"><p>${fb.originalContent.feedback}</p></div><div class="feedback-date">Originally Posted: ${new Date(fb.originalContent.timestamp).toLocaleString()}</div><div style="margin-top:auto;"><button class="flip-btn" onclick="flipCard('${fb._id}')">VIEW EDITED</button></div></div>`;
                }
                html += `</div></div>`;
            }
        }
        html += `</div><div id="adminModalOverlay" class="admin-modal-overlay"><div class="admin-custom-modal"><h3 id="adminModalTitle"></h3><p id="adminModalMessage"></p><div class="admin-modal-buttons"><button id="adminModalOkButton">OK</button><button id="adminModalConfirmButton" style="display:none;">Confirm</button><button id="adminModalCancelButton" style="display:none;">Cancel</button></div></div></div><script>const AUTH_HEADER = '${authHeaderValue}'; if (!AUTH_HEADER || AUTH_HEADER === "Basic Og==") { console.error("CRITICAL: AUTH_HEADER is missing or invalid in admin panel script!"); showAdminModal('alert', 'Authentication Error', 'Admin authentication is not configured properly. Actions will fail.'); } const adminModalOverlay = document.getElementById('adminModalOverlay'), adminModalTitle = document.getElementById('adminModalTitle'), adminModalMessage = document.getElementById('adminModalMessage'), adminModalOkButton = document.getElementById('adminModalOkButton'), adminModalConfirmButton = document.getElementById('adminModalConfirmButton'), adminModalCancelButton = document.getElementById('adminModalCancelButton'); let globalConfirmCallback = null; function showAdminModal(type, title, message, confirmCallbackFn = null) { adminModalTitle.textContent = title; adminModalMessage.textContent = message; globalConfirmCallback = confirmCallbackFn; adminModalOkButton.style.display = type === 'confirm' ? 'none' : 'inline-block'; adminModalConfirmButton.style.display = type === 'confirm' ? 'inline-block' : 'none'; adminModalCancelButton.style.display = type === 'confirm' ? 'inline-block' : 'none'; adminModalOverlay.style.display = 'flex'; } adminModalOkButton.addEventListener('click', () => adminModalOverlay.style.display = 'none'); adminModalConfirmButton.addEventListener('click', () => { adminModalOverlay.style.display = 'none'; if (globalConfirmCallback) globalConfirmCallback(true); }); adminModalCancelButton.addEventListener('click', () => { adminModalOverlay.style.display = 'none'; if (globalConfirmCallback) globalConfirmCallback(false); }); function flipCard(id) { document.getElementById(\`card-\${id}\`).classList.toggle('is-flipped'); } async function tryDeleteFeedback(id) { console.log("Attempting to delete feedback ID:", id); showAdminModal('confirm', 'Delete Feedback?', 'Are you sure you want to delete this feedback? This cannot be undone.', async confirmed => { if (confirmed) { try { const res = await fetch(\`/api/admin/feedback/\${id}\`, { method: 'DELETE', headers: { 'Authorization': AUTH_HEADER } }); if (res.ok) { showAdminModal('alert', 'Deleted!', 'Feedback deleted successfully.'); setTimeout(() => location.reload(), 1000); } else { const err = await res.json(); console.error("Delete failed response:", err); showAdminModal('alert', 'Error!', \`Failed to delete: \${err.message || res.statusText}\`); } } catch (e) { console.error("Delete fetch error:", e); showAdminModal('alert', 'Fetch Error!', \`Error during delete: \${e.message}\`); } } }); } async function tryPostReply(fbId, txtId) { const replyText = document.getElementById(txtId).value.trim(); console.log("Attempting to post reply to feedback ID:", fbId, "Text:", replyText); if (!replyText) { showAdminModal('alert', 'Empty Reply', 'Please write something to reply.'); return; } showAdminModal('confirm', 'Post Reply?', \`Confirm reply: "\${replyText.substring(0,50)}..."\`, async confirmed => { if (confirmed) { try { const res = await fetch(\`/api/admin/feedback/\${fbId}/reply\`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': AUTH_HEADER }, body: JSON.stringify({ replyText, adminName: '👉𝙉𝙊𝘽𝙄𝙏𝘼🤟' }) }); if (res.ok) { showAdminModal('alert', 'Replied!', 'Reply posted.'); setTimeout(() => location.reload(), 1000); } else { const err = await res.json(); console.error("Reply failed response:", err); showAdminModal('alert', 'Error!', \`Failed to reply: \${err.message || res.statusText}\`); } } catch (e) { console.error("Reply fetch error:", e); showAdminModal('alert', 'Fetch Error!', \`Error during reply: \${e.message}\`); } } }); } async function tryChangeUserAvatar(userId, userName) { console.log("Attempting to change avatar for user ID:", userId, "Name:", userName); showAdminModal('confirm', 'Change Avatar?', \`Change avatar for \${userName}? This will regenerate avatar for this email user.\`, async confirmed => { if (confirmed) { try { const res = await fetch(\`/api/admin/user/\${userId}/change-avatar\`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': AUTH_HEADER } }); if (res.ok) { showAdminModal('alert', 'Avatar Changed!', 'Avatar updated for ' + userName + '.'); setTimeout(() => location.reload(), 1000); } else { const err = await res.json(); console.error("Change avatar failed response:", err); showAdminModal('alert', 'Error!', \`Failed to change avatar: \${err.message || res.statusText}\`); } } catch (e) { console.error("Change avatar fetch error:", e); showAdminModal('alert', 'Fetch Error!', \`Error during avatar change: \${e.message}\`); } } }); } function toggleSelectAll(checked) { document.querySelectorAll('.feedback-checkbox').forEach(checkbox => checkbox.checked = checked); } async function tryDeleteSelectedFeedbacks() { const selectedFeedbackIds = Array.from(document.querySelectorAll('.feedback-checkbox:checked')).map(cb => cb.value); if (selectedFeedbackIds.length === 0) { showAdminModal('alert', 'No Feedbacks Selected', 'Please select at least one feedback to delete.'); return; } showAdminModal('confirm', 'Delete Selected Feedbacks?', \`Are you sure you want to delete \${selectedFeedbackIds.length} selected feedback(s)? This cannot be undone.\`, async confirmed => { if (confirmed) { try { const res = await fetch('/api/admin/feedbacks/batch-delete', { method: 'DELETE', headers: { 'Content-Type': 'application/json', 'Authorization': AUTH_HEADER }, body: JSON.stringify({ ids: selectedFeedbackIds }) }); if (res.ok) { showAdminModal('alert','Deleted!',\`\${selectedFeedbackIds.length} feedback(s) deleted successfully.\`); setTimeout(()=>location.reload(),1000); } else { const err = await res.json(); console.error("Batch delete failed response:",err); showAdminModal('alert','Error!',\`Failed to delete selected feedbacks: \${err.message||res.statusText}\`); } } catch (e) { console.error("Batch delete fetch error:",e); showAdminModal('alert', 'Fetch Error!', \`Error during batch delete: \${e.message}\`); } } }); }</script></body></html>`;
        res.send(html);
    } catch (error) {
...
app.delete('/api/admin/feedbacks/batch-delete', authenticateAdmin, async (req, res) => {
    ...
    if (!Array.isArray(ids) || ids.length === 0) return res.status(400).json({ message: 'No IDs provided for deletion.' });
    try {
