...
// LOGIN route mein email verification check ko hataya gaya hai
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        // --- EMAIL VERIFICATION CHECK LOGIN PAR HATAYA GAYA HAI ---
        // ताकि unverified users bhi login kar saken
        ...
        if (!user) {
            ...
            if (user) {
                if (user.loginMethod === 'email') {
                    ...
                    user.isVerified = true; // Google se login karne par email account verify ho jayega
                }
            } else {
                user = new User({
                    googleId,
                    ...
                    loginMethod: 'google',
                    isVerified: true // Google accounts by default verified mane jayenge
                });
            }
            await user.save();
        } else {
...
app.get('/api/auth/verify-email', async (req, res) => {
    ...
    try {
        ...
        // Redirection to the index page with verification success status and new token
        // This token is passed to the frontend to automatically log the user in if needed
        const userForToken = { userId: user._id, name: user.name, email: user.email, avatarUrl: user.avatarUrl, loginMethod: user.loginMethod, isVerified: user.isVerified };
        const appToken = jwt.sign(userForToken, JWT_SECRET, { expiresIn: '7d' });

        return res.redirect(`${FRONTEND_URL}/index.html?verification=success&token=${appToken}`);
...
app.get('/api/feedbacks', async (req, res) => {
    try {
        // userId field ko populate karte samay 'isVerified' bhi select karein
